...
static const musique_info_t tabMusiques[] = { // sera remplacé plus tard par la lecture réelle de fichiers
...
chargerMusiques_result_t chargerMusiques_get(arraylist_t **musiques) {
	...
	const size_t nbFichiers = sizeof(tabMusiques) / sizeof(tabMusiques[0]);
	for (size_t i = 0; i < nbFichiers; ++i) {
		const musique_info_t *elem = &tabMusiques[i];
		chargerMusiques_result_t resMU = ajouterMusique(elem->nomFichier, musiques);
		if (resMU != CHARGERMUSIQUES_OK) return resMU;
	}
